...
def get_db_model_fields():
    import time # 전부 요청 받는데 몇초 걸리는지 트래킹
    ...
    for i in tqdm(range(1, 4)):
        tmp = get(redis_client, f'SIMPLI_WORKER_{str(i)}_TICKERS_LIST')
        tmp = [[d[0], d[3]] for d in tmp]
        tickerlist += tmp
    end = time.time()
    print(f'Ticker list: {end - start}s')
    ...
    #     try:
    #         ticker = tickerlist[i]
    #         price_list = get(redis_client, f'SIMPLI_{ticker[1]}_{ticker[0]}_PRICE_LIST')
    #         price_data = price_list[0]
    #         price_keys = price_data.keys()
    ...
    # end = time.time()
    # print(f'Price keys: {end - start}s')
    # print(price_fields)

    # STEP 2: dividend fields
    start = time.time()
    for i in tqdm(range(len(tickerlist))):
        try:
            ticker = tickerlist[i]
            exchange = ticker[1]
            if exchange != 'INDX':
                div_list = get(redis_client, f'SIMPLI_{ticker[1]}_{ticker[0]}_DIVIDEND_LIST')
                if len(div_list) > 0:
                    div_data = div_list[0]
                    div_keys = div_data.keys()
                    for key in div_keys:
                        dividend_fields.add(key)
        except:
            print(div_list)
            print(traceback.format_exc())
    end = time.time()
    print(f'Dividend keys: {end - start}s')
    print(dividend_fields)
        

    # fundamental_json = get(redis_client, 'SIMPLI_US_AAPL_FUNDAMENTAL_JSON')

    # print(fundamental_json.keys())
    # print(fundamental_json['General'].keys())
    # print(fundamental_json['General']['Code'].keys())
